# Archived Operations, Recovery, Upgrades, and Troubleshooting

> Historical evidence only. This version includes the retired VM test path;
> use the current `docs/operations.md` replacement.
...
For the short, copy-and-run production procedure, see
...
* **Lab:** stage a local intermediate once (for example from the preproduction
...
> your root/intermediate constrains (for example an `aigw.test`-constrained
...
### One image release workflow (`scripts/update-images.py`)

The old per-service script was retired. It had fixed lab host and inventory
defaults, did not run a mandatory generic acceptance gate, and could suggest a
binary rollback after a one-way state change.

The replacement has three explicit commands: `prepare`, `test-preprod`, and
`upgrade`. It builds one schema-v2 seed containing every external pin and every
custom/preprod image. It can deploy those exact image IDs to local Ansible
preprod. A remote upgrade requires a clean previous source checkout, the
previous seed, an independently mounted encrypted backup destination, and the
matching controller-held age identity. Any deploy or validation failure
restores the authenticated state backup, converges the previous source and
seed, and reruns validation. It never falls back to image tags alone.

See [Image update workflow](image-update-workflow.md) for complete commands and
the fail-closed rollback rules.
