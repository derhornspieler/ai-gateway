# Archived Identity, Directory Federation, and Group Operations

> Historical evidence only. This version includes the retired VM test identity
> path; use the current `docs/identity-operations.md` replacement.
...
- [One-time identity-controller bootstrap](#one-time-identity-controller-bootstrap)
  — [Prerequisites](#prerequisites) · [Deployment procedure](#deployment-procedure)
...
A generic customer deployment does not start Samba. It creates or reconciles
the one inventory-defined customer LDAPS provider automatically when the normal
Ansible playbook runs. `samba-ad` remains a lab-only fixture and must never be
treated as a customer directory. The customer provider uses hostname-verified
TLS, a least-privilege bind identity, `READ_ONLY` edit mode, imported users, and
no portal access to the bind credential.
...
On the first deployment, Keycloak's temporary master-realm bootstrap service
client (`aigw-bootstrap-controller`, `KC_BOOTSTRAP_ADMIN_CLIENT_ID`) must be
available. Its secret exists only in the encrypted deployment overlay and the
Keycloak and rotator container environments; it never reaches a portal or a
browser. Ansible consumes it after the durable controller is verified.
...
There is no browser initialization step. Provide the LDAPS settings and bind
credential through the Ansible inventory, initialize and unseal Vault, and run
the normal playbook. The `docker_stack` role runs the idempotent identity setup
inside `key-rotator`. The playbook fails unless it verifies the LDAP federation,
durable controller, break-glass escrow, Vault OIDC escrow, callback URLs, and
removal of the temporary administrator.

After the playbook succeeds, open `https://admin.<domain>` as an administrator.
The portal displays readiness booleans and certificate fingerprints, but it has
no setup button. Record the fingerprints in the deployment evidence and compare
them after a restore. If setup fails, fix the reported Ansible error and rerun
the same playbook.

#### What happens internally (for troubleshooting and audit)

The Ansible-run rotator command calls one authenticated loopback route in the
running one-worker service. That route shares the bootstrap lock and performs
one ordered transaction, consuming the temporary bootstrap service client and
establishing durable controls:
...
**When to run:** once, after the Ansible identity setup has completed on a
stack deployed with the current converge — the escrow at
`kv/ai-gateway/keycloak/vault-oidc-rp` and
...
`kv/data/ai-gateway/keycloak/vault-oidc-rp`), restore the temporary service
client if needed, and rerun the Ansible playbook. The automatic identity step
writes the escrow and reconciles the `vault` client. Then run
`scripts/vault-oidc-setup.sh`. The ceremony fails closed with a pointed message
if the escrow is missing.

## Domain migration on an existing realm

Changing `aigw_domain` after the realm database already exists (for example
migrating `old.aigw.test` to `new.aigw.test`) does **not** re-key the
...
The normal Ansible identity command repairs this automatically on both new and
existing deployments. It calls the single locked deployment route. During the
first setup that route uses the temporary bootstrap authority; after setup it
uses the Vault-escrowed break-glass administrator. It changes only each managed
client's callback, origin, and logout allow-list, reads every value back, and
fails the converge if anything does not match `aigw_domain`. The durable
controller still has no `manage-clients` role. No browser action, separate
redirect script, or standing controller grant is needed.
...
3. Restore the temporary service client as described below, then rerun the
   Ansible playbook. The automatic setup sees the missing escrow and disabled
   account, sets a fresh generated password, verifies the escrow write, and
...
3. **Rerun Ansible**: perform the stopped-Keycloak temporary-service recovery
   below, then run the normal playbook. Its automatic identity step provisions
   the group, master brute-force policy, and break-glass user, and escrows the
   credential.
...
rerun the normal Ansible playbook; the automatic setup reuses and proves valid
state, repairs mismatched keys
...
rerun Ansible to retry the bounded cleanup. Do not delete an unmarked client
because its name looks similar, and
...
**What success looks like:** the Ansible playbook completes with verified
state, the temporary client is deleted, and status no
...
callback), `test-portal-identity-flow.py` (OIDC step-up and proof that automatic
identity setup is complete), `test-portal-group-flow.py` (create/assign/remove/delete under real
...
imports only into an empty database. The automatic Ansible identity step uses
the deployment's controlled administrator to reconcile the `admin-ui` client's
`post.logout.redirect.uris` before it reports success. If that verification
fails, the converge stops rather than shipping a broken Grafana sign-out that
lands on Keycloak's "Invalid redirect uri" error page. The verified value is
the four admin host roots, `##`-joined and each with a trailing slash:

```
https://litellm-admin.<domain>/##https://grafana.<domain>/##https://prometheus.<domain>/##https://vault.<domain>/
```

The deployment command writes and reads back this exact value. Do not patch it
manually in the browser or with `kcadm`; correct `aigw_domain` in inventory and
rerun Ansible.
