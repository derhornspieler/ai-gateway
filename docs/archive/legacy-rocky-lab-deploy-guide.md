...
> Historical evidence only. This version includes the retired VM test profile;
> use `docs/deploy-runbook.md` for production and `docs/preprod.md` locally.
...
That file pins `community.docker` 5.2.1, `community.general` 13.2.0, and
`ansible.posix` 2.2.2.
...
Identity setup has no browser initialization step. Supply the LDAPS connection,
CA, bind DN, and bind-password file in the encrypted inventory, then run the
normal Ansible converge. The running one-worker key-rotator automatically
creates or reconciles the federation, deploys the Vault-backed controller,
aligns every OIDC callback with `aigw_domain`, removes only marked temporary
principals, and verifies the final state. On the lab/preprod profile, the
inventory-owned pre-Vault baseline also creates the declared groups and assigns
the Samba users. For production, declare the approved directory administrators
and group policy before converge; do not use a Keycloak bootstrap login or an
admin-portal setup button. See [identity operations](identity-operations.md)
