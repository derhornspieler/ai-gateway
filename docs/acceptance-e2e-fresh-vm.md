...
|---|---|---|---|
| `chat.` | ADM (`traefik-adm`) | open-webui | chat UI is an operator surface |
...
| Domain / DNS | `aigw.aegisgroup.ch`; `platform_authoritative_dns_enabled: true` (in-stack CoreDNS) | real `<INTERNAL_DNS_ZONE>`; `platform_authoritative_dns_enabled: false` — engineer creates the **DNS records** (`chat/admin/…` → ADM IP, `api/portal/auth` → internal IP) in the corporate zone |
