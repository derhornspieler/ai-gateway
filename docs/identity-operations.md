...
8. provision the durable, group-gated master-realm administrator (every
   profile): create or adopt the marked `keycloak-admins` group and map
   master's composite `admin` role onto it, apply the same pinned brute-force
   policy the imported realms carry (master is never file-imported, so
   Keycloak's default leaves it off), and create the marked `break-glass-admin`
   user **disabled**, set a rotator-generated password, escrow it to Vault with
   a verified write, and only then enable the account — any failure disables
   the account and fails the bootstrap (see
   [Break-glass administrator](#break-glass-administrator));
...
token is read from stdin so it never appears on a command line or in shell
history):

```bash
cd /opt/ai-gateway
read -rs VAULT_TOKEN && export VAULT_TOKEN
scripts/aigw-compose.sh exec -e VAULT_TOKEN vault \
...
delete the escrowed version, then re-run the idempotent initialization through
the temporary-service recovery below. The ensure step sees the missing escrow,
disables the account, sets a fresh generated password, escrows it with a
verified write, and re-enables the account. Never change the break-glass
password in the console by hand — that leaves the Vault escrow stale.
