...
if [[ "$runtime_reconcile" == true ]]; then
    ...
    membership_count="$(psql --username "$POSTGRES_USER" --dbname postgres \
        --tuples-only --no-align --command "
          ...
          WHERE granted_role.rolname IN ('litellm','keycloak','rotator')
             OR member_role.rolname IN ('litellm','keycloak','rotator');")"
    [[ "$membership_count" == 0 ]] || state_changed=true

    structure_ok="$(psql --username "$POSTGRES_USER" --dbname postgres \
        --tuples-only --no-align --command \
        "SELECT (SELECT count(*) FROM pg_roles WHERE rolname IN ('litellm','keycloak','rotator')) = 3 AND
...
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" \
     ...
     -v fix_connect_acl="$fix_connect_acl" <<-'EOSQL'
