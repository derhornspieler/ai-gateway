...
Read CLAUDE.md first. Inference path: clients → traefik-int (api.<domain>, allow-listed inference paths only) → LiteLLM (virtual keys, master key custody in Vault) → Envoy egress at 172.28.0.2 (plain HTTP to http://envoy-egress:8080/<vendor>/..., Envoy originates pinned TLS) → vendor. key-rotator drives vendor credential rotation (anthropic_wif.py, openai_svcacct.py) and pushes into LiteLLM's credential API.
...
- Vendor API surface goes through Envoy path-prefix routing only; never add a direct vendor URL to a service. New vendor endpoints mean Envoy route + CA bundle + firewall review, not a config shortcut.
