#!/usr/bin/env python3
"""Store one stdin-supplied directory bind credential as an inline Ansible Vault value.

This helper is intentionally narrow: it writes only
``identity_ldap_bind_password``, requires the operator to name the destination
and Vault identity explicitly, and never accepts the credential in argv or the
environment.  The recommended destination is the dedicated sibling overlay
``<generated-inventory>/group_vars/generic_rocky9/identity-ldap.yml``.

The credential belongs to a read-only customer directory service account. It is
never randomly generated here, exactly like ``vault_unseal_key``.
"""
...
VARIABLE_NAME = "identity_ldap_bind_password"
VAULT_ID_RE = re.compile(r"[A-Za-z0-9][A-Za-z0-9_.-]{0,63}\Z")
# Printable ASCII, 16-512 bytes, no leading/trailing whitespace, no control
# characters. Ansible renders this value into a root-owned secret file and the
# key-rotator sends it as an LDAP simple-bind credential; a control character
# or an unbounded value would cross those boundaries ambiguously.
BIND_PASSWORD_RE = re.compile(rb"[\x21-\x7e][\x20-\x7e]{14,510}[\x21-\x7e]\Z")
VARIABLE_RE = re.compile(r"(?m)^\s*identity_ldap_bind_password\s*:")
...
def arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Read one external directory bind credential from stdin and "
            "atomically store it as the inline-encrypted Ansible variable "
...
def validate_inputs(args: argparse.Namespace) -> tuple[Path, Path, bytes]:
    ...
    if sys.stdin.isatty():
        fail(
            "pipe exactly one bind credential on stdin; interactive input is disabled"
        )
    raw = bytearray(sys.stdin.buffer.read(MAX_INPUT_BYTES + 1))
    if len(raw) > MAX_INPUT_BYTES:
        for index in range(len(raw)):
            raw[index] = 0
        fail("bind credential input exceeds the bounded maximum")
    if raw.endswith(b"\r\n"):
        del raw[-2:]
    elif raw.endswith(b"\n"):
        raw.pop()
    if BIND_PASSWORD_RE.fullmatch(raw) is None:
        for index in range(len(raw)):
            raw[index] = 0
        fail(
            "stdin did not contain exactly one 16-512 byte printable-ASCII bind "
            "credential without leading or trailing whitespace"
        )
    value = bytes(raw)
    for index in range(len(raw)):
        raw[index] = 0
...
def read_existing(destination: Path) -> tuple[str, bool]:
    if not destination.exists():
        return (
            (
                "---\n"
                "# Operator-custodied external directory material. Values in this "
                "file must remain inline-encrypted.\n"
            ),
            False,
        )
    try:
    ...
    if VARIABLE_RE.search(existing):
        fail(
            "identity_ldap_bind_password already exists; refusing replacement or "
...
def encrypt_value(args: argparse.Namespace, password_file: Path, value: bytes) -> str:
    ...
    if result.returncode != 0:
        fail(
            "ansible-vault could not encrypt identity_ldap_bind_password; check "
            "the explicit Vault ID and password file"
        )
    try:
...
def main() -> int:
    try:
        ...
        return 2
    print(
        f"Stored and verified inline-encrypted identity_ldap_bind_password in: {destination}"
    )
    print(
        "Keep identity_ldap_enabled=false until every identity_ldap_* host-var is "
        "complete; enabling it changes the firewall ABI and requires a full site.yml."
    )
    return 0
